...
export function StepUpload({ onDone }: { onDone: (imp: CsvImport) => void }) {
  ...
  const upload = useMutation({
    mutationFn: async () => {
      if (!accountId || !file) throw new Error('Sélectionne un compte et un fichier CSV.');
  ...
  return (
    <div className="space-y-6">
      <section>
        <label className="block text-sm font-medium mb-2">Compte de destination</label>
        <select
          value={accountId}
      ...
      <section>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition ${
            isDragging
              ? 'border-cat-teal bg-cat-teal-bg'
        ...
        >
          ...
          {file ? (
            <div>
              ...
              <div className="text-xs text-cat-teal-fg mt-2">Cliquer pour en choisir un autre</div>
            </div>
          ) : (
            <div className="text-sm text-gray-500">
              Glisse-dépose ton fichier CSV ici, ou clique pour choisir.
            </div>
          )}
        </div>
      </section>
      ...
      <div className="flex justify-end">
        <button
          ...
          className="px-4 py-2 bg-cat-teal-fg text-white rounded-md text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed hover:bg-cat-teal-fg/90"
        >
